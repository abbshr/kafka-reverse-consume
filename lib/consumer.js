...
var Consumer = function (client, topics, options) {
  ...
  this.olderOffset = this.options.olderOffset;
  this.ready = false;
...
Consumer.prototype.buildPayloads = function (payloads) {
  ...
  return payloads.map(function (p) {
    ...
    // console.log("building payload", p);
    return p;
  });
};
...
Consumer.prototype.init = function () {
  ...
  self.client.topicExists(topics, function (err) {
    ...
    // console.log("init payload:", self.payloads);
    self.fetchOffset(self.payloads, function (err, topics) {
      if (err) {
        return self.emit('error', err);
      }
      // console.log("init fetchoffset", topics);
      self.updateOffsets(topics, true);
      // console.log("init updateoffset payload:", self.payloads);
      self.fetch();
    });
  });
};

Consumer.prototype.reverseOffsets = function (topics) {
  var self = this;
  var no_proceed = false;
  this.payloads.forEach(function (p) {
    if (!_.isEmpty(topics[p.topic]) && topics[p.topic][p.partition] !== undefined) {
      if (p.offset <= self.olderOffset) return no_proceed = true;
...
Consumer.prototype.fetch = function (no_proceed) {
  if (this.reverse && no_proceed) return;
